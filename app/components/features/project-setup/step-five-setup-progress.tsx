          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            {!brand || ((!brand.brand_assets || brand.brand_assets.length === 0) && !isTaskCompleted) ? (
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                {onAssetUpload && (
                  <label className={`group relative flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 p-2 transition-colors hover:border-[#2f6f71] hover:bg-white ${isUploadingAsset ? 'pointer-events-none opacity-60' : ''}`}>
                    <input
                      type="file"
                      accept="image/*"
                    ...
                    <div className="flex h-24 w-full flex-col items-center justify-center gap-2 rounded-md">
                      {isUploadingAsset ? (
                        <svg
                          className="h-8 w-8 animate-spin text-[#2f6f71]"
              ...
              <div className="mt-2 space-y-3">
                ...
                {onAssetUpload && (
                  <label className={`group inline-flex cursor-pointer items-center gap-2 rounded-lg border border-slate-300 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition-colors hover:border-[#2f6f71] hover:bg-white hover:text-[#2f6f71] ${isUploadingAsset ? 'pointer-events-none opacity-60' : ''}`}>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={handleFileUpload}
